...
function initTowhub() {
 ...
 // scroll animation ------------------
    ...
    $('.close-reg , .reg-overlay').on("click", function () {
	    ...
	    var login = {};
    login.hide = function () {
        $('.login , .log-overlay').fadeOut(200);
        $("html, body").removeClass("hid-body");
        $(".modal_main").removeClass("vis_mr");
    };
    $('.modal-open1').on("click", function (e) {
        e.preventDefault();
        $('.login , .log-overlay').fadeIn(200);
        $(".modal_main").addClass("vis_mr");
        $("html, body").addClass("hid-body");
    });
	
    $('.close-log, .log-overlay').on("click", function () {
        login.hide();
	});
...
function initAutocomplete() {
  ...
  var acInputs = document.getElementsByClassName("autocomplete-input");
    ...
    for (var i = 0; i < acInputs.length; i++) {
        var autocomplete1 = new google.maps.places.Autocomplete(acInputs[i], options);
...
// Currency Formatting Jquery Dependency 
